...
**SPEC-002 entregada:** modelo zod en `src/model/` (vocabulario de estados, cualificadores, reglas y alertas), migraciones base en `supabase/migrations/` (ocho tablas: competitions, teams, team_aliases, matches, observations, decisions, alerts, ingest_attempts; vista `board` de solo lectura; RLS en todas las tablas; extensiones pg_cron y pg_net para ingesta), test de arquitectura con Vitest (`test:db`).
